...
class SystemsIndexer extends IndexerBase
{
    ...
    public function customIndexer(array &$indexerConfig, IndexerRunner &$indexerObject): string
    {
        ...
        foreach ($systems as $record) {
            ...
            $Application  = strip_tags($record['Application']);
            $properties  = strip_tags($record['properties']);
            $abstract = strip_tags($record['description']);
            $sysgid = file_get_contents("https://api.westwood.de/api/system/$id");
            

            $fullContent = $title . "\n" . $subtitle . "\n" . $description . "\n" . $Application . "\n" . $properties;

            // Link to detail view
            $params = '&tx_wwapi_api[system_id]=' . $record['id'] . '&tx_wwapi_api[group]=' . $sysgid . '&tx_wwapi_api[controller]=Item';
            ...
            $indexerObject->storeInIndex(
                ...
                $title,                         // record title
                SystemsIndexer::KEY,            // content type
