...
class ApplicationIndexer extends IndexerBase
{
    ...
    public function customIndexer(array &$indexerConfig, IndexerRunner &$indexerObject): string
    {
        ...
        foreach($applications as $record) {
            ...
            // Compile the information, which should go into the index.
                ...
                $header = strip_tags($record['header']);
                $highlights_title = strip_tags($record['highlights_title']);
                $highligts = strip_tags($record['highligts']);
                $description  = strip_tags($record['description']);

                $fullContent = $title . "\n" . $abstract . "\n" . $header . "\n" .$highlights_title . "\n" .$highligts . "\n" . $description;

                // Link to detail view
                $params = '&tx_wwapi_api[application_id]='.$record['id'].'&tx_wwapi_api[controller]=Item';
                ...
                $indexerObject->storeInIndex(
                    ...
                    $title,                         // record title
                    ApplicationIndexer::KEY,            // content type
